# TiendaPay CLI — Pear Track submission

Entrega para el track "Pears" (Tether) de Aleph Hackathon 2026: la CLI real de [TiendaPay](../README.md) empaquetada como binario standalone e instalable P2P con `pear install`, con actualizaciones OTA.

## De dónde parte

Arranca desde el boilerplate oficial [`hello-pear-bare`](https://github.com/holepunchto/hello-pear-bare), rama **`variant/daemon`** — pensada para comandos cortos tipo git (el proceso corre y termina mientras un daemon desprendido chequea updates en background), que es exactamente el patrón de uso de TiendaPay (`invoice create`, `pay`, etc.), a diferencia de un TUI/servicio de larga vida.

Se mantuvo intacto el mecanismo de updates del boilerplate (`bin.mjs` / `app.js`, basado en `pear-runtime` + `bare-daemon`). Lo que se agregó es el CLI real de TiendaPay (`src/`, portado 1:1 desde la raíz del repo) conectado como el comando que corre después de que el updater arranca en background.

## Qué incluye (y qué no)

Incluye todo TiendaPay **excepto el asistente `ask` (QVAC)**:
...
tiendapay-cli --no-updates help
tiendapay-cli --no-updates wallet generate-seed
```
...
El binario resultante (`out/linux-x64/tiendapay`) se copia a `deploy/by-arch/<host>/app/tiendapay-cli` — esa es la estructura de carpetas que `pear install` espera (`/by-arch/<plataforma>-<arch>/app/<name>`, con `<name>` = el campo `"name"` de `deploy/package.json`). Se stagea parado en `deploy/` (que tiene su propio `package.json`, separado del de desarrollo) para no subir accidentalmente `node_modules`/`src` al drive — solo el binario final.
...
**Validado de punta a punta contra una instalación real.** Con `tiendapay-cli` instalado vía `pear install` en `0.0.4`, se stageó una `0.0.5` nueva en el mismo link, y se corrió el daemon de update sobre la copia instalada (`tiendapay-cli --updater --storage <dir> --update-window 150000`). Log real:

```
info  [updater] getting new update
info  [updater] { op: 'add', key: '/by-arch/linux-x64/app/tiendapay-cli', bytesAdded: 119309064 }
info  [updater] update complete... applying
info  [updater] applied update, restart to run latest version
```

`tiendapay-cli --version` pasó de `0.0.4` a `0.0.5` solo, en ~2 segundos una vez que arrancó el chequeo — nada de lentitud de red real.
...
2. **`productName` vs `name` en `package.json`**: `bin.mjs` armaba el nombre del updater con `pkg.productName` ("tiendapay"), pero `pear install` arma la ruta del binario en el drive con `pkg.name` ("tiendapay-cli") — el updater buscaba `/by-arch/linux-x64/app/tiendapay` cuando el archivo real vivía en `/by-arch/linux-x64/app/tiendapay-cli`, y tiraba `Error: update not found`. Se arregló usando `pkg.name` consistentemente en `bin.mjs`.
